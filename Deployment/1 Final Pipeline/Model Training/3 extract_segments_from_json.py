...
def main():
    ...
    for key, info in intervals.items():
        ...
        group = info.get('group', 'UNK')  # If not found, use UNK
        ...
        # breathing → diagnosis label (Healthy/Crackle/Rhonchi/Wheezing etc.)
        if diag and len(breathing)>0:
            label = diag if diag in LABELS_ALLOWED or diag=='Healthy' else diag
            # Record abnormal labels as is (can be mapped in post-processing)
        ...
        # non_breathing → "Non-breathing" label
