...
# ─────────────────────────────────────────────────────────
# Model Adapter (window 2.0s / hop 1.0s)
# ─────────────────────────────────────────────────────────
class RealtimeModelAdapter:
    ...
    def _load_models_build_reorder(self):
        ...
        for c in self.UI_ORDER:
            key = c  # 'Non-breathing' included same display
            try:
...
class WearableStethoUI(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Wearable Stethoscope — UI")
        ...
        self.btn_import.clicked.connect(self.play_from_file)  # Top Import: file import playback
        self.btn_music.clicked.connect(self.play_from_file)   # Original Music: file playback

        self.adapter = RealtimeModelAdapter(MODEL_DIR)
        self.adapter.reset_stream()

        # Audio player (actual sound output when file import playback)
    ...
    def _build_ui(self):
        ...
        # Section 1: Top controls  (Import button next to Save)
        ...
        self.btn_import = QtWidgets.QPushButton("Import")  # additional
        ...
        sec1.addWidget(self.btn_import)  # next to Save
        sec1.addStretch()
        ...
        # Section 4: Interactive Feedback
    ...
    def handle_data(self, handle, data):
        ...
        # 4kHz → 16kHz upsampling → 2s window / 1s hop inference
    ...
    def play_from_file(self):
        ...
        # visualization/resampling for model
        if sr != SAMPLE_RATE:
        ...
        # timer(50ms) for floating/supply chunk to model
    ...
    def _playback_tick(self):
        ...
        # for total saving (based on 4k time)
        start_time = self.time_counter - shift_len / SAMPLE_RATE
        full_time_array = start_time + np.arange(shift_len) / SAMPLE_RATE
        self.full_time.extend(full_time_array)
        self.full_audio.extend(chunk4)

        # push 16k to model adapter (2s window / 1s hop)
