#!/usr/bin/env python3
"""
Generate UMAP visualizations for clustering results for B0_new: NoSeg + PeakNormalize + HighPass
"""
...
def create_umap_visualization(features_df, representation_name, clustering_results, output_dir):
    ...
    for algo_key, result in clustering_results.items():
        ...
        # Customize plot
        if algorithm == 'kmeans':
            title = f"B0_new: {representation_name} - K-Means (k={result['k']})"
            filename = f"umap_{representation_name}_kmeans_k{result['k']}.png"
        else:  # hdbscan
            title = f"B0_new: {representation_name} - HDBSCAN (min_cluster_size={result['min_cluster_size']})"
...
def main():
    ...
    print("="*60)
    print("B0_new: NoSeg + PeakNormalize + HighPass - Visualizations")
    print("="*60)
