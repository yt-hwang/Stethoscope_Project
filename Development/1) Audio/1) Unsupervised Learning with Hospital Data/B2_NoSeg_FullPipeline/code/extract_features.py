#!/usr/bin/env python3
"""
Cycle B2: NoSeg + PeakNormalize + Bandpass + SpectralGating - Feature Extraction
Complete pipeline combining all best-performing A-Series methods
"""
...
def apply_peak_normalization(audio, target_db=-20):
    """Apply peak normalization to consistent loudness level"""
    # Convert to dB
...
def process_audio_file(file_path, sr=16000):
    """Process a single audio file with complete preprocessing pipeline"""
    try:
        # Load audio
        audio, orig_sr = librosa.load(file_path, sr=sr, mono=True)
        
        # Apply complete preprocessing pipeline: PeakNormalize + Bandpass + SpectralGating
        # Step 1: Peak normalization for consistent loudness
        audio_normalized = apply_peak_normalization(audio, target_db=-20)
        
        # Step 2: Bandpass filter (100-2000 Hz)
        audio_bandpass = apply_bandpass_filter(audio_normalized, sr, low_freq=100, high_freq=2000)
        
        # Step 3: Spectral gating for noise reduction
        ...
        features = {
            ...
            'preprocessing': 'peak_normalize_bandpass_100_2000_spectral_gating',
            **raw_stats
        }
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for Cycle B2')
    ...
    print(f"\nFeature extraction completed for Cycle B2: NoSeg + PeakNormalize + Bandpass + SpectralGating")
