...
def load_clustering_results(output_dir):
    ...
    for rep in representations:
        metrics_path = os.path.join(output_dir, f'clustering_{rep}_metrics.json')
...
def create_metrics_rows(cycle_id, all_results, output_dir):
    ...
    for rep_name, results in all_results.items():
        for algo_name, metrics in results.items():
            ...
            features_csv = os.path.join(output_dir, f'features_{rep_name}.csv')
            metrics_json = os.path.join(output_dir, f'clustering_{rep_name}_metrics.json')
            umap_png = os.path.join(output_dir, f'umap_{rep_name}_{algo_name}.png')
...
def main():
    ...
    cycle_summary_row = create_cycle_summary_row(
        ...
        notes='Baseline cycle with no preprocessing'
    )
    
    # Create metrics rows
    metrics_rows = create_metrics_rows('A0', all_results, args.output_dir)
