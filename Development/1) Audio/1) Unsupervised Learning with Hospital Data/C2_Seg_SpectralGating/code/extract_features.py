#!/usr/bin/env python3
"""
Cycle C2: Seg + SpectralGating - Feature Extraction
Segmentation with spectral gating for noise reduction
"""
...
def process_audio_file(file_path, sr=16000):
    """Process a single audio file with segmentation and spectral gating"""
    try:
        # Load audio
        audio, orig_sr = librosa.load(file_path, sr=sr, mono=True)
        
        # Apply spectral gating for noise reduction
        audio_processed = apply_spectral_gating(audio, sr, threshold_percentile=10, alpha=0.1)
        ...
        features = {
            ...
            'preprocessing': 'segmented_10sec_spectral_gating',
            **raw_stats
        }
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for Cycle C2')
    ...
    print(f"\nFeature extraction completed for Cycle C2: Seg + SpectralGating")
