#!/usr/bin/env python3
"""
C1: Seg + Bandpass - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Apply segmentation (10s windows) + bandpass filtering before feature extraction
"""
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            ...
            segment_processed = bandpass_filter(segment, sr, low_freq=100, high_freq=2000)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for C1: Seg + Bandpass - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/C1_Seg_Bandpass/outputs',
    ...
    print("NEW DATA EXPERIMENT - C1: Seg + Bandpass")
    print("=" * 60)
    print(f"Dataset: RAW sound_ML test sound list")
    print(f"Found {len(audio_files)} audio files")
    print(f"Segmentation: 10-second windows")
    print(f"Preprocessing: Bandpass filter (100-2000 Hz)")
    print()
