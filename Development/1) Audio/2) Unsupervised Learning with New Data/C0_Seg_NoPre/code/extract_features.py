#!/usr/bin/env python3
"""
C0: Seg + NoPreprocess - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Apply segmentation (10s windows) with NO preprocessing - pure segmentation baseline
This will show the pure effect of segmentation vs A0!
"""
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            # NO PREPROCESSING - C0 segmentation baseline
            
            # Extract features from raw segment
            raw_stats = extract_raw_waveform_stats(segment, sr)
            logmel_mean = extract_logmel_mean(segment, sr)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for C0: Seg + NoPreprocess - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/C0_Seg_NoPre/outputs',
    ...
    print("NEW DATA EXPERIMENT - C0: Seg + NoPreprocess")
    print("=" * 60)
    print(f"Dataset: RAW sound_ML test sound list")
    print(f"Found {len(audio_files)} audio files")
    print(f"Segmentation: 10-second windows")
    print(f"Preprocessing: NONE (pure segmentation baseline)")
    print("Testing pure segmentation effect vs A0!")
    print()
