#!/usr/bin/env python3
"""
C4: Seg + PeakNormalize - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Apply segmentation (10s windows) + peak normalization before feature extraction
TESTING SEGMENTATION EFFECT ON A4 (TIED WINNER - 0.360)!
"""
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            # PREPROCESSING: Apply peak normalization
            segment_processed = peak_normalize(segment)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for C4: Seg + PeakNormalize - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/C4_Seg_PeakNormalize/outputs',
    ...
    print("NEW DATA EXPERIMENT - C4: Seg + PeakNormalize")
    print("=" * 60)
    print(f"Dataset: RAW sound_ML test sound list")
    print(f"Found {len(audio_files)} audio files")
    print(f"Segmentation: 10-second windows")
    print(f"Preprocessing: Peak normalization")
    print("TESTING SEGMENTATION EFFECT ON A4 (TIED WINNER - 0.360)!")
    print()
