#!/usr/bin/env python3
"""
B2: NoSeg + FullPipeline - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Apply complete preprocessing pipeline: PeakNormalize + Bandpass + SpectralGating + HighPass
"""
...
def bandpass_filter(audio, sr, low_freq=100, high_freq=2000, order=5):
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        # PREPROCESSING: Apply FULL PIPELINE
        # Step 1: Peak normalization
        audio_processed = peak_normalize(audio)
        # Step 2: Bandpass filter
        audio_processed = bandpass_filter(audio_processed, sr, low_freq=100, high_freq=2000)
        # Step 3: Spectral gating
        audio_processed = spectral_gating(audio_processed, sr)
        # Step 4: High-pass filter
        audio_processed = highpass_filter(audio_processed, sr, cutoff_freq=20)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for B2: NoSeg + FullPipeline - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/B2_NoSeg_FullPipeline/outputs',
