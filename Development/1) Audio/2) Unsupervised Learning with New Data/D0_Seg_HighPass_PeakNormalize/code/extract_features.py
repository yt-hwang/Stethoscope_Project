#!/usr/bin/env python3
"""
D0: Seg + HighPass + PeakNormalize - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Combine segmentation with amplitude + frequency domain methods:
- HighPass20 (best segmentation compatibility: -3.6% penalty)
- PeakNormalize (A4 winner: 0.360 NoSeg)
TESTING: Can amplitude + frequency combination beat D1's frequency-only combo?
"""
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            # PREPROCESSING: Apply HighPass + PeakNormalize (amplitude + frequency combination)
            segment_highpass = apply_highpass_filter(segment, sr, cutoff_freq=20)
            segment_processed = peak_normalize(segment_highpass)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for D0: Seg + HighPass + PeakNormalize - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/D0_Seg_HighPass_PeakNormalize/outputs',
    ...
    print("NEW DATA EXPERIMENT - D0: Seg + HighPass + PeakNormalize")
    print("=" * 65)
    print(f"Dataset: RAW sound_ML test sound list")
    print(f"Found {len(audio_files)} audio files")
    print(f"Segmentation: 10-second windows")
    print(f"Preprocessing: HighPass(20Hz) + PeakNormalize")
    print("AMPLITUDE + FREQUENCY COMBINATION TEST!")
    print("Testing: Can amplitude+frequency beat D1's frequency-only combo?")
    print()
