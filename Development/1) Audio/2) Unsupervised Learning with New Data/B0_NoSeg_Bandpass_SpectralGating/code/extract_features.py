#!/usr/bin/env python3
"""
B0: NoSeg + Bandpass + SpectralGating - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Apply bandpass filtering (100-2000 Hz) + spectral gating before feature extraction
"""
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        # PREPROCESSING: Apply bandpass filter + spectral gating
        audio_processed = bandpass_filter(audio, sr, low_freq=100, high_freq=2000)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for B0: NoSeg + Bandpass + SpectralGating - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/B0_NoSeg_Bandpass_SpectralGating/outputs',
    ...
    print("NEW DATA EXPERIMENT - B0: NoSeg + Bandpass + SpectralGating")
    print("=" * 60)
    print(f"Dataset: RAW sound_ML test sound list")
    print(f"Found {len(audio_files)} audio files")
    print(f"Preprocessing: Bandpass (100-2000 Hz) + Spectral gating")
    print()
