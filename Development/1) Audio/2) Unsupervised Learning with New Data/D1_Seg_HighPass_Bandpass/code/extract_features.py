#!/usr/bin/env python3
"""
D1: Seg + HighPass + Bandpass - Feature Extraction
NEW DATA EXPERIMENT - Using RAW sound_ML test sound list
Combine segmentation with the two best frequency-domain methods:
- HighPass20 (best segmentation compatibility: -3.6% penalty)
- Bandpass (good performance: 0.349 NoSeg)
TESTING: Can frequency-domain combination beat individual C-series?
"""
...
def process_audio_file(file_path, target_sr=16000):
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            # PREPROCESSING: Apply HighPass + Bandpass (frequency-domain combination)
            segment_highpass = apply_highpass_filter(segment, sr, cutoff_freq=20)
            segment_processed = apply_bandpass_filter(segment_highpass, sr, low_freq=100, high_freq=2000)
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for D1: Seg + HighPass + Bandpass - New Data Experiment')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/Unsupervised Learning with New Data/D1_Seg_HighPass_Bandpass/outputs',
    ...
    print("NEW DATA EXPERIMENT - D1: Seg + HighPass + Bandpass")
    print("=" * 60)
    print(f"Dataset: RAW sound_ML test sound list")
    print(f"Found {len(audio_files)} audio files")
    print(f"Segmentation: 10-second windows")
    print(f"Preprocessing: HighPass(20Hz) + Bandpass(100-2000Hz)")
    print("FREQUENCY-DOMAIN COMBINATION TEST!")
    print("Testing: C3 winner + C1 method = better than individuals?")
    print()
