#!/usr/bin/env python3
"""
Cycle D2: Seg + Full Pipeline - Feature Extraction
Complete preprocessing: PeakNormalize + Bandpass + SpectralGating + HighPass
"""
...
def bandpass_filter(y, sr, lowcut=100, highcut=2000, order=5):
...
def spectral_gating(y, sr, prop_decrease=0.8, freq_mask_smooth_hz=50, time_mask_smooth_ms=50, n_fft=N_FFT, hop_length=HOP_LENGTH):
...
def highpass_filter(y, sr, cutoff=20, order=5):
...
def extract_logmel_mean(y, sr):
    """Extract log-mel spectrogram mean features."""
...
def main():
    parser = argparse.ArgumentParser(description="Extract features from segmented audio with complete preprocessing pipeline.")
    parser.add_argument('--audio_dir', type=str, required=True, 
                       help='Path to the segmented audio directory.')
    parser.add_argument('--output_dir', type=str, required=True, 
                       help='Path to the output directory for features.')
    
    args = parser.parse_args()

    # Find all WAV files in the segmented audio directory
    audio_files = []
    ...
    for i, file_path in enumerate(audio_files):
        ...
        try:
            ...
            # Step 3: Spectral gating (noise reduction)
            y_processed = spectral_gating(y_processed, sr)
            
            # Step 4: High-pass filter (20 Hz)
            y_processed = highpass_filter(y_processed, sr, cutoff=20)
    ...
    print(f"\nFeature extraction completed for {len(audio_files)} files")
