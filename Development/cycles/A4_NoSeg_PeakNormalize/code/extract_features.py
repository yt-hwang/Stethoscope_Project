#!/usr/bin/env python3
"""
Cycle A4: NoSeg + PeakNormalize - Feature Extraction
"""
...
def apply_peak_normalization(audio):
    """Apply peak normalization to scale audio to [-1, 1]"""
...
def process_audio_file(file_path, sr=16000):
    ...
    try:
        ...
        audio_normalized = apply_peak_normalization(audio)
        
        # Extract features
        raw_stats = extract_raw_waveform_stats(audio_normalized)
        logmel_mean = extract_logmel_mean(audio_normalized, sr)
        mfcc_mean = extract_mfcc_mean(audio_normalized, sr)
        
        # Combine all features
        features = {
            'file_path': str(file_path),
            'filename': Path(file_path).name,
            'preprocessing': 'peak_normalize',
            **raw_stats
        }
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for Cycle A4')
    ...
    parser.add_argument('--output_dir', 
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/cycles/A4_NoSeg_PeakNormalize/outputs',
    ...
    print(f"\nFeature extraction completed for Cycle A4: NoSeg + PeakNormalize")
