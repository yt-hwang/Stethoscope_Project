#!/usr/bin/env python3
"""
Feature extraction for Cycle A1: NoSeg + Bandpass(100-2000 Hz)
Extracts raw_waveform_stats, logmel_mean, and mfcc_mean representations
with bandpass filtering applied
"""
...
def apply_bandpass_filter(audio, sr, lowcut=100, highcut=2000, order=4):
...
def process_audio_file(file_path, target_sr=16000, lowcut=100, highcut=2000):
    """Process a single audio file with bandpass filtering and extract all features"""
    try:
        ...
        raw_stats = extract_raw_waveform_stats(filtered_audio, sr)
        logmel_mean = extract_logmel_mean(filtered_audio, sr)
        mfcc_mean = extract_mfcc_mean(filtered_audio, sr)
        
        # Create feature dictionary
        features = {
            'file_path': file_path,
            'filename': os.path.basename(file_path),
            'preprocessing': f'bandpass_{lowcut}_{highcut}',
            **raw_stats
        }
...
def main():
    parser = argparse.ArgumentParser(description='Extract features for Cycle A1 with bandpass filtering')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/Users/yunhwang/Desktop/Stethoscope_Project/Development/cycles/A1_NoSeg_Bandpass/outputs',
                       help='Path to output directory')
    parser.add_argument('--lowcut', type=int, default=100,
                       help='Low cutoff frequency for bandpass filter')
    parser.add_argument('--highcut', type=int, default=2000,
    ...
    for i, file_path in enumerate(audio_files):
        ...
        features = process_audio_file(file_path, lowcut=args.lowcut, highcut=args.highcut)
