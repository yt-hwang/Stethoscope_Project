#!/usr/bin/env python3
"""
Extract features from non-segmented audio files with PeakNormalize + HighPass preprocessing.
This combines the best A-Series individual methods: A4 (PeakNormalize) + A3 (HighPass)
"""
...
def highpass_filter(y, sr, cutoff_freq=20, order=5):
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract features from non-segmented audio files with PeakNormalize + HighPass preprocessing."
    )
    parser.add_argument('--audio_dir', type=str, required=True,
                        help='Path to the root directory containing non-segmented audio files.')
    ...
    for i, file_path in enumerate(audio_files):
        ...
        try:
            ...
            # Apply preprocessing: Peak Normalize + High-pass Filter
            y_processed = peak_normalize(y)
            y_processed = highpass_filter(y_processed, sr, cutoff_freq=20)
