#!/usr/bin/env python3
"""
CNN Model for 5-Class Abnormal Breathing Classification USING SCALOGRAMS

This version uses Continuous Wavelet Transform (CWT) scalograms instead of mel-spectrograms.
Scalograms provide better time-frequency resolution for breathing sounds, especially:
- Crackles (short, sharp transients)
- Wheezing (varying frequency patterns)
- Rhonchi (low-frequency continuous sounds)
...
import librosa
import pywt  # PyWavelets for scalogram generation
...
class Config:
    ...
    OUTPUT_DIR = Path('/Users/yunhwang/Desktop/Stethoscope_Project/Development/2) Image/CNN_Abnormal_Breathing/Results_Scalogram')
    ...
    SCALOGRAM_HEIGHT = 64      # Number of scales (frequency resolution)
    SCALOGRAM_WIDTH = 128      # Time resolution
    WAVELET = 'morl'           # Morlet wavelet (good for audio)
    SCALES = np.logspace(1, 3, SCALOGRAM_HEIGHT)  # Logarithmic scale range
    
    # CNN parameters
    INPUT_HEIGHT = SCALOGRAM_HEIGHT  # Scalogram scales
    INPUT_WIDTH = SCALOGRAM_WIDTH    # Time frames
    NUM_CLASSES = 5
...
class ScalogramGenerator:
    ...
    def create_scalogram(self, audio: np.ndarray, sr: int) -> np.ndarray:
        """Create scalogram from audio using CWT."""
        try:
            ...
            # Get magnitude (absolute value of complex coefficients)
            scalogram = np.abs(coefficients)
            
            # Resize to target dimensions
            scalogram = self._resize_scalogram(scalogram)
            
            # Normalize to [0, 1]
            if scalogram.max() != scalogram.min():
                scalogram = (scalogram - scalogram.min()) / (scalogram.max() - scalogram.min())
            
            return scalogram
            
        except Exception as e:
            print(f"Error creating scalogram: {e}")
            # Return zero scalogram if CWT fails
            return np.zeros((self.config.SCALOGRAM_HEIGHT, self.config.SCALOGRAM_WIDTH))
    
    def _resize_scalogram(self, scalogram: np.ndarray) -> np.ndarray:
        """Resize scalogram to target dimensions."""
        from scipy import ndimage
        
        target_height = self.config.SCALOGRAM_HEIGHT
        target_width = self.config.SCALOGRAM_WIDTH
        
        # Current dimensions
        current_height, current_width = scalogram.shape
        ...
        resized = ndimage.zoom(scalogram, (zoom_y, zoom_x), order=1)
        
        return resized

# =========================
# Data Augmentation for Scalograms
# =========================

class ScalogramAugmentation:
    """Augmentation techniques for scalograms."""
    ...
    def time_stretch(self, audio: np.ndarray, sr: int, factor: float = None) -> np.ndarray:
        ...
        if factor is None:
            factor = np.random.uniform(*self.config.TIME_STRETCH_RANGE)
    ...
    def time_masking(self, scalogram: np.ndarray, max_mask_size: int = 10) -> np.ndarray:
        """Apply time masking to scalogram."""
        masked = scalogram.copy()
    ...
    def frequency_masking(self, scalogram: np.ndarray, max_mask_size: int = 8) -> np.ndarray:
        """Apply frequency masking to scalogram."""
        masked = scalogram.copy()
    ...
    def augment_audio(self, audio: np.ndarray, sr: int) -> List[np.ndarray]:
        ...
        # Time stretching
        stretched = self.time_stretch(audio, sr)
        augmented_samples.append(stretched)
        
        # Pitch shifting
        shifted = self.pitch_shift(audio, sr)
        augmented_samples.append(shifted)
        
        # Noise addition
        noisy = self.add_noise(audio)
...
class ScalogramBreathingDataset(Dataset):
    """PyTorch Dataset for breathing audio classification using scalograms."""
    
    def __init__(self, file_list: List[str], labels: List[int], audio_dir: Path, 
                 target_sr: int = 4000, input_width: int = 128, input_height: int = 64,
        ...
        # Initialize scalogram generator and augmentation
        self.scalogram_gen = ScalogramGenerator(Config)
        self.augmenter = ScalogramAugmentation(Config)
    ...
    def _create_augmented_dataset(self):
        """Create augmented dataset."""
        print(f"Creating augmented scalogram dataset with factor {self.augmentation_factor}...")
    ...
    def __getitem__(self, idx):
        ...
        # Create scalogram
        scalogram = self.scalogram_gen.create_scalogram(audio, sr)
        
        # Apply scalogram-level augmentations
        if is_augmented and self.augment:
            # Randomly apply masking
            if np.random.random() < 0.5:
                scalogram = self.augmenter.time_masking(scalogram)
            if np.random.random() < 0.5:
                scalogram = self.augmenter.frequency_masking(scalogram)
        
        # Convert to tensor and add channel dimension
        scalogram_tensor = torch.FloatTensor(scalogram).unsqueeze(0)  # Shape: (1, scales, time_frames)
...
# =========================
# CNN Model Architecture (optimized for scalograms)
# =========================
...
class ScalogramCNN(nn.Module):
    """CNN model optimized for scalogram input (5-class breathing classification)."""
    
    def __init__(self, input_height: int = 64, input_width: int = 128, num_classes: int = 5, dropout_rate: float = 0.3):
        super(ScalogramCNN, self).__init__()
        
        self.input_height = input_height
        self.input_width = input_width
        self.num_classes = num_classes
        
        # Convolutional layers (optimized for scalogram characteristics)
...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, 
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), Config.OUTPUT_DIR / 'best_model_scalogram.pth')
        else:
            patience_counter += 1
...
def plot_training_history(history: Dict[str, List[float]], output_dir: Path):
    ...
    ax1.set_title('Training and Validation Loss (Scalogram CNN)')
    ...
    ax2.set_title('Training and Validation Accuracy (Scalogram CNN)')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'training_history_scalogram.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_confusion_matrix(cm: np.ndarray, class_names: List[str], output_dir: Path):
    ...
    plt.title('Confusion Matrix - 5-Class CNN with Scalograms')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.tight_layout()
    plt.savefig(output_dir / 'confusion_matrix_scalogram.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def save_results(results: Dict[str, Any], output_dir: Path):
    ...
    report_df.to_csv(output_dir / 'classification_report_scalogram.csv')
    
    # Save detailed results
    with open(output_dir / 'evaluation_results_scalogram.pkl', 'wb') as f:
        pickle.dump(results, f)
    
    # Print summary
    print("\n" + "="*60)
    print("EVALUATION RESULTS SUMMARY (SCALOGRAM CNN)")
    print("="*60)
...
def main():
    """Main function to run the complete pipeline with scalograms."""
    print("="*60)
    print("CNN 5-Class Abnormal Breathing Classification USING SCALOGRAMS")
    print("="*60)
    ...
    # Create datasets with scalograms
    print(f"\nCreating scalogram datasets...")
    train_dataset = ScalogramBreathingDataset(X_train, y_train, Config.AUDIO_DIR, augment=True)
    val_dataset = ScalogramBreathingDataset(X_val, y_val, Config.AUDIO_DIR, augment=False)
    test_dataset = ScalogramBreathingDataset(X_test, y_test, Config.AUDIO_DIR, augment=False)
    ...
    print(f"  Scales: {len(Config.SCALES)} (logarithmic range)")
    
    # Train model
    print(f"\nStarting training with scalograms...")
    history = train_model(model, train_loader, val_loader, Config.NUM_EPOCHS, 
                         Config.LEARNING_RATE, Config.DEVICE)
    
    # Load best model for evaluation
    model.load_state_dict(torch.load(Config.OUTPUT_DIR / 'best_model_scalogram.pth'))
    ...
    torch.save({
        ...
        'scalogram_config': {
            ...
            'scales': Config.SCALES.tolist(),
            'augmentation_factor': Config.AUGMENTATION_FACTOR
        }
    }, Config.OUTPUT_DIR / 'complete_model_scalogram.pth')
    
    print(f"\nResults saved to: {Config.OUTPUT_DIR}")
    print("Training with scalograms completed successfully!")
