#!/usr/bin/env python3
# train_tl_img_breathing_efficientnet.py
# Breathing vs NonBreathing 이진 분류 (EfficientNet-B0, GroupKFold by patient)
...
# ===== 경로/설정 =====
MANIFEST = Path("/Users/yunhwang/Desktop/Stethoscope_Project/Development/9.2) Reference/BreathingTiles_mel/manifest.csv")
OUTDIR   = MANIFEST.parent / "models_efficientnet_b0_binary"
OUTDIR.mkdir(parents=True, exist_ok=True)

CLASSES = ["NonBreathing", "Breathing"]  # 0,1
LABELS_IDX = [0,1]
...
def counts_by_class(df: pd.DataFrame):
    vc = df.label.value_counts().reindex(CLASSES, fill_value=0)
    return vc.to_dict()

def print_split_stats(name: str, df: pd.DataFrame):
    by_cls = counts_by_class(df)
    patients = df.patient_id.nunique()
    total = len(df)
    pretty = ", ".join([f"{c}: {by_cls[c]}" for c in CLASSES])
    print(f"{name} -> n={total}, patients={patients} | [{pretty}]")
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=2.0):
        super().__init__()
        self.alpha = alpha  # class weight tensor on device or None
        self.gamma = gamma
...
def evaluate(dl, model, device):
    ...
    for x,y,_ in dl:
        ...
        tot += ce.mean().item()*x.size(0); n += x.size(0)
    ...
    # AUPRC(positive=Breathing=1). 클래스 한쪽만 있으면 예외 발생 → None 처리
    try:
        auprc = average_precision_score(y, p[:,1])
    except Exception:
        auprc = None
    return tot/n, f1, auprc, y, pred, p

def train_fold(train_df, val_df, fold=0, device="cuda"):
    tr = ImgDS(train_df, train=True); va = ImgDS(val_df, train=False)
    ...
    # 클래스 가중치(선택)
    counts = train_df.label.value_counts().reindex(CLASSES, fill_value=0).values
    # focal용 alpha: inverse-frequency 정규화
    alpha = counts.astype(float)
    alpha = (alpha.sum()/np.maximum(alpha,1e-6)).astype(np.float32)
    alpha = alpha * (len(alpha)/alpha.sum())
    alpha = torch.tensor(alpha, dtype=torch.float32).to(device)

    # BCE pos_weight (Breathing=1 양성)
    pos_weight = None
    if counts.sum()>0 and counts[1]>0:
    ...
    if LOSS_TYPE.lower()=="bce":
        def criterion(logits, target):
            return F.binary_cross_entropy_with_logits(
                logits[:,1], (target==1).float(), pos_weight=pos_weight
            )
    else:  # focal
        fl = FocalLoss(alpha=alpha, gamma=2.0)
        def criterion(logits, target):
            return fl(logits, target)
    ...
    def run_epoch(dl, train=True):
        ...
        try:
            auprc = average_precision_score(y_true, p[:,1])
        except Exception:
            auprc = None
        return tot/n, f1, auprc
    ...
    def safe_load(pth, map_location=None):
        try:
            return torch.load(pth, map_location=map_location, weights_only=True)
        except TypeError:
