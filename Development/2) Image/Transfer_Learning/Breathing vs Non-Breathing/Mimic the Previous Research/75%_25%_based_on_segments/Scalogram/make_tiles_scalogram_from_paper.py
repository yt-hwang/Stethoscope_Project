#!/usr/bin/env python3
# make_tiles_scalogram_from_paper.py
# 논문식 전처리(HPF + Wavelet denoise) → 2s/0.5s 타일 → midpoint 라벨 + 경계 ±0.3s 드롭
# Morlet CWT 스케일로그램 224x224 저장 (vmin/vmax 퍼센타일 스케일링)
...
def build_scales(sr: int, wavelet: str, fmax: float, num_scales: int):
    cf = pywt.central_frequency(wavelet)  # Morlet central freq
...
def main():
    ...
    for fname, rec in meta.items():
        ...
        while s0 + win <= len(y):
            ...
            show = scalo_db  # FMAX 이내는 scales로 제한됨
            t0 = s0 / SR
