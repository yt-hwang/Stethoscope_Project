...
# === Aggregation helpers (summary across folds) ===
reports_all: list = []
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
        ...
        # === accumulate per-fold classification reports for summary ===
        try:
            ...
            print(f"[WARN] Could not accumulate fold {fold} reports: {_e}")
    ...
    # === Aggregated classification-report summary across folds ===
    try:
        if len(reports_all) > 0:
            rep_df = pd.concat(reports_all, ignore_index=True)
            rep_df.to_csv(os.path.join(summary_dir, "classification_report_by_fold.csv"),
            ...
            macro_weighted_df = summary_df[summary_df['label'].isin(['macro avg','weighted avg'])].copy()
            ...
            for col in ['precision', 'recall', 'f1-score']:
                macro_rows[col] = pd.to_numeric(macro_rows[col], errors='coerce').fillna(0.0)
            ...
            if not best_overall.empty:
                ...
                lines.append(f"[BEST OVERALL] fold={int(r['fold'])}, version={r['version']}, "
                             f"macroF1={r['f1-score']:.4f}, macroP={r['precision']:.4f}, macroR={r['recall']:.4f}")
            for _, r in best_by_version.iterrows():
                lines.append(f"[BEST by VERSION] {r['version']}: fold={int(r['fold'])}, "
                             f"macroF1={r['f1-score']:.4f}, macroP={r['precision']:.4f}, macroR={r['recall']:.4f}")

            with open(os.path.join(summary_dir, "best_fold_selection.txt"), "w", encoding="utf-8") as f:
                f.write("\n".join(lines) + "\n")
        else:
            print("[INFO] No fold-level classification reports to aggregate.")
    except Exception as _e:
        print(f"[WARN] Failed to aggregate classification reports: {_e}")
