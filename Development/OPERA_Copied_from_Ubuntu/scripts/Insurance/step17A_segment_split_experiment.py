...
"""
Step 17A: Competition-Grade Data Augmentation (SEGMENT-BASED SPLIT VERSION)

- Fixed BatchNorm issue with small batches
- Already achieving excellent results: 0.444, 0.537!
- Feature-level augmentation optimized for OPERA embeddings
- Maintains LDAM+DRW pipeline (current best practice)
- CHANGED: Uses segment-based split instead of patient-based split
"""
...
from sklearn.model_selection import StratifiedKFold  # CHANGED: from StratifiedGroupKFold to StratifiedKFold
...
class CompetitionLinearModelStable(nn.Module):
    ...
    def __init__(self, input_dim, num_classes, dropout=0.2):
        ...
        # FIXED: Use LayerNorm instead of BatchNorm for stability
        self.feature_norm = nn.LayerNorm(input_dim)  # Works with any batch size
    ...
    def forward(self, x):
        # FIXED: LayerNorm works with batch size 1
...
def train_one_epoch(model, loader, device, optimizer, criterion, criterion_ldam,
    ...
    for x_batch, y_batch in loader:
        ...
        if aug_choice < 0.4:  # 40% Mixup
            x_batch, y_a, y_b, lam = mixup_data(x_batch, y_batch, alpha=DEF_MIXUP_ALPHA)
            mixed = 'mixup'
            mixup_count += 1
        elif aug_choice < 0.4 + DEF_CUTMIX_PROB:  # 30% CutMix
...
def main():
    ...
    print("=" * 70)
    print("🔬 SEGMENT-BASED SPLIT EXPERIMENT VERSION")
    print("=" * 70)
    print("⚠️  CHANGED: Using StratifiedKFold instead of StratifiedGroupKFold")
    print("⚠️  This splits by segments, NOT by patients!")
    print("⚠️  Use this as backup comparison only!")
    print("=" * 70)
    ...
    # REMOVED: Patient groups - not needed for segment-based split
    ...
    # Print warning about segment-based split
    print("")
    print("⚠️  WARNING: Using SEGMENT-BASED split!")
    print("⚠️  This may lead to data leakage as segments from same patient")
    print("⚠️  can appear in both training and validation sets!")
    print("⚠️  Use only as backup comparison experiment!")
    print("")
    ...
    # Cross-validation - CHANGED: from StratifiedGroupKFold to StratifiedKFold
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=args.seed)  # CHANGED
    tau_grid = np.linspace(args.tau_min, args.tau_max, args.tau_steps)

    rows = []
    # CHANGED: removed groups parameter from split
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):  # CHANGED
        print(f"\n{'='*20} Fold {fold} {'='*20}")
        ...
        # Print split info for segment-based approach
        ...
        # Training loop
        print(f"  Training with segment-based split...")
        best_mr = -1.0
        patience = 0
        ...
        # Final evaluation and enhanced tau search
        ...
        print(f"  FOLD {fold} RESULTS (SEGMENT-SPLIT):")
        ...
        rows.append({
            ...
            'split_method': 'SEGMENT_BASED',  # CHANGED: new field to track split method
            'augmentation_strategy': 'Competition_Grade_SEGMENT_SPLIT',
    ...
    print("\n" + "=" * 70)
    print("🔬 SEGMENT-BASED SPLIT EXPERIMENT RESULTS")
    print("=" * 70)
    ...
    print("\n⚠️  REMEMBER: This is SEGMENT-BASED split experiment!")
    print("⚠️  May have data leakage - use as backup comparison only!")
    print("⚠️  For publication, use patient-based split!")
