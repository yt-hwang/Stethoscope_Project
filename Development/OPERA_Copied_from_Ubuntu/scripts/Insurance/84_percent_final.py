...
- [ADDED] Save checkpoints per fold: model_best.pt / model_last.pt (+ scaler, class_names, best_tau)
"""
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 8 == 0 or epoch == 1 or epoch == args.epochs:
                ...
                recs_raw_tmp = per_class_recall(y_true_va, y_pred_raw_tmp, C)
                mr_raw_tmp = float(np.mean(recs_raw_tmp))

                lr = scheduler.get_last_lr()[0]
                print(f"  Epoch {epoch:03d}: lr={lr:.2e} loss={train_loss:.4f} val_MR={mr_raw_tmp:.3f}")

                # [ADDED] Track best by Macro Recall on validation (raw)
        ...
        # [ADDED] Save "last" model before overwriting/using best
        ...
        # [ADDED] Load best state for final eval & saving "best" checkpoint
        if best_state_dict is not None:
            model.load_state_dict(best_state_dict)
            print(f"  🔁 Restored best checkpoint from epoch {best_epoch} (val MR={best_mr:.3f})")
        else:
            print("  ⚠️ No best_state_dict captured; proceeding with last epoch weights.")

        # Final evaluation with OPTIMIZED tau search (using best checkpoint)
        ...
        # [ADDED] Save best checkpoint (after tau/metrics so best_tau included)
        best_ckpt = {
            "state_dict": model.state_dict() if best_state_dict is None else best_state_dict,
            "input_dim": X.shape[1],
            "num_classes": C,
            "class_names": class_names,
            "scaler": scaler,                 # pickled inside .pt
            "best_tau": best_tau.tolist(),    # final per-class tau
            "seed": actual_seed,
        ...
        # [ADDED] Write a small info file
        with open(os.path.join(fold_dir, "model_info.txt"), "w", encoding="utf-8") as f:
            f.write(f"Best epoch: {best_epoch}\n")
            f.write(f"Best val Macro Recall (raw): {best_mr:.4f}\n")
            f.write(f"Macro Recall (raw end): {mr_raw:.4f}\n")
            f.write(f"Macro Recall (tau): {mr_tau:.4f}\n")
    ...
    print("  ✅ Checkpoints: model_best.pt / model_last.pt per fold")
