...
RESULTS_DIR  = os.path.join(PROJECT_DIR, 'results', 'Ensemble_Thresh_ADAPTIVE')
...
print('[RUN] Step11R2 Ensemble + Adaptive NB-gate + Guard relax')
...
def apply_thresholds_with_gate(P, tauH, tauW, tauR, nb_gate):
    y_pred = np.full(P.shape[0], -1, np.int64)
    nb_mask = P[:, IDX_NB] >= nb_gate
    y_pred[nb_mask] = IDX_NB
    if (~nb_mask).any():
        Q = P[~nb_mask].copy()
        tau = np.full(Q.shape[1], 0.5, np.float32)
        tau[IDX_H] = tauH; tau[IDX_W] = tauW; tau[IDX_R] = tauR
        y_pred[~nb_mask] = (Q / tau.reshape(1,-1)).argmax(axis=1)
    return y_pred

# ---------- Outer CV ----------
gkf = GroupKFold(n_splits=5)
ALL_T, ALL_P, ALL_PR = [], [], []
ALL_FILE, ALL_PID, ALL_FOLD, TAUS, GATES = [], [], [], [], []
fold=0
...
for tr, te in gkf.split(X_all, y_all, groups_all):
    ...
    # probs on val for tuning + gate calibration
    P_lr_val, _, _  = infer_lr(X_val)
    P_mlp_val, _, _ = infer_mlp(X_val)
    P_val = ensemble_probs(P_lr_val, P_mlp_val, 0.5, 0.5)

    # baseline (no gate, tau=0.5)
    base_pred = P_val.argmax(axis=1)
    base_nb = recall_score(y_val, base_pred, labels=[IDX_NB], average=None, zero_division=0)[0]

    # ---- adaptive NB gate selection on val ----
    # choose smallest gate in [0.30..0.50] s.t. NB recall on val >= max(base_nb, 0.45)
    target_guard = max(base_nb, 0.45)
    cand_gates = np.arange(0.30, 0.51, 0.01)
    chosen_gate = cand_gates[-1]
    for g in cand_gates:
        y_hat = apply_thresholds_with_gate(P_val, 0.5, 0.5, 0.5, g)
        nb_rec = recall_score(y_val, y_hat, labels=[IDX_NB], average=None, zero_division=0)[0]
        if nb_rec + 1e-9 >= target_guard:
            chosen_gate = g; break
    print(f"[GATE] baseline_NB={base_nb:.3f} → guard={target_guard:.3f} → chosen_gate={chosen_gate:.2f}")

    # ---- τ grid with guard relax if needed ----
    grid = np.arange(0.30, 0.71, 0.05)
    best = (-1.0, 0.5, 0.5, 0.5)
    guard = target_guard
    for relax in range(0, 8):  # relax guard down by 0.02 up to 7 times
        found = False
        for tH in grid:
            for tW in grid:
                for tR in grid:
                    yhat = apply_thresholds_with_gate(P_val, tH, tW, tR, chosen_gate)
                    ...
                    if nb_rec + 1e-9 < guard:  # NB must stay above guard
                        continue
                    found = True
                    if macro > best[0]:
                        best = (macro, tH, tW, tR)
        if found: break
        guard = max(base_nb, guard - 0.02)  # relax toward baseline but not below
    if best[0] < 0:
        print("[TUNE] no τ met guard → fallback τ=(0.5,0.5,0.5)")
        best = (0.0, 0.5, 0.5, 0.5)

    print(f"[TUNE] val macro*={best[0]:.3f} with tau_H={best[1]:.2f} tau_W={best[2]:.2f} tau_R={best[3]:.2f} | guard≥{guard:.3f}")
    TAUS.append((best[1], best[2], best[3])); GATES.append(chosen_gate)
    ...
    y_pred = apply_thresholds_with_gate(P_te, best[1], best[2], best[3], chosen_gate)
...
print("\n--- Aggregate Classification Report (Ensemble + ADAPTIVE) ---")
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       xlabel='Predicted', ylabel='True', title='Ensemble + ADAPTIVE Threshold (Aggregate)')
plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, 'ensemble_adaptive_cm.png'), dpi=150); plt.close(fig)
...
pd.DataFrame(GATES, columns=['nb_gate']).to_csv(os.path.join(RESULTS_DIR, 'nb_gates_per_fold.csv'), index=False)
...
pd.DataFrame([{
    'model': 'Ensemble(LogReg+MLP)+AdaptiveGate+GuardRelax',
