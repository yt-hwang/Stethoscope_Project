...
RESULTS_DIR  = os.path.join(PROJECT_DIR, 'results', 'Ensemble')
...
print('[RUN] Step8 Ensemble (TorchLogReg + MLP)'); print(f'[PATH] {CSV_PATH}')
...
def train_logreg(X_tr, y_tr, X_te, batch=64, epochs=30, lr=1e-3):
    ...
    crit = FocalLoss(alpha=alpha_t, gamma=1.0)
    ...
    for ep in range(1, epochs+1):
        ...
        if ep==1 or ep%5==0:
            print(f"[LR EP {ep:02d}] loss={tot/len(ds_tr):.4f}")

    # probs on test
    ds_te = NumpyDataset(X_te, np.zeros(len(X_te)))
    dl_te = build_loader(ds_te, batch, sampler=None, shuffle=False)
    model.eval(); probs=[]
    with torch.no_grad():
        for xb, _, _, _ in dl_te:
            p = torch.softmax(model(xb.to(device)), dim=1).cpu().numpy()
            probs.append(p)
    return scaler, model, np.vstack(probs)

def train_mlp(X_tr, y_tr, X_te, batch=64, epochs=50, lr=1e-3, weight_decay=1e-4):
    n_class = len(class_names)
    scaler = StandardScaler()
    X_tr = scaler.fit_transform(X_tr).astype(np.float32)
    X_te = scaler.transform(X_te).astype(np.float32)
    ...
    for ep in range(1, epochs+1):
        ...
        for xb, yb, _, _ in dl_tr:
            ...
            loss.backward(); opt.step(); tot += loss.item()*xb.size(0)
        if ep==1 or ep%5==0:
            print(f"[MLP EP {ep:02d}] loss={tot/len(ds_tr):.4f}")
...
for tr, te in gkf.split(X_all, y_all, groups_all):
    ...
    # train both models on the same train fold
    _, lr_model, P_lr = train_logreg(X_tr, y_tr, X_te, batch=64, epochs=30, lr=1e-3)
    _, mlp_model, P_mlp = train_mlp(X_tr, y_tr, X_te, batch=64, epochs=50, lr=1e-3, weight_decay=1e-4)

    # average probs
    P_ens = 0.5 * P_lr + 0.5 * P_mlp
    y_pred = P_ens.argmax(axis=1)

    ALL_T.append(y_te); ALL_P.append(y_pred); ALL_PR.append(P_ens)
...
print("\n--- Aggregate Classification Report (Ensemble) ---")
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       title='Ensemble (LogReg + MLP) Aggregate')
plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, 'ensemble_cm_aggregate.png'), dpi=150); plt.close(fig)
...
pred_df.to_csv(os.path.join(RESULTS_DIR, 'ensemble_cv_predictions.csv'), index=False)
...
pd.DataFrame([{
    'model': 'Ensemble[LogReg(Focal g=1.0) + MLP(256,Focal g=1.0)]',
