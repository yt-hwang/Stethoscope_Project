...
RESULTS_DIR  = os.path.join(PROJECT_DIR, 'results', 'MLP_Focal_FeatureAug')
...
print('[RUN] Step9 MLP + Focal + Feature-space Aug (mixup+noise)'); print(f'[PATH] {CSV_PATH}')
...
def mixup_batch(x, y, alpha=0.3, class_weights=None):
    """
    x: [B,D], y: [B], returns mixed x, soft labels not used (we keep hard labels by primary index)
    class_weights: per-sample draw prob for partner index (bias minority)
    """
    if alpha <= 0: return x, y
    lam = np.random.beta(alpha, alpha)
    B = x.size(0)
    # choose partners with (optional) class-aware prob
    if class_weights is None:
        idx = torch.randperm(B, device=x.device)
    else:
        # class_weights is np.array per class → per-sample probs
        # map each sample's partner by multinomial over sample-level weights
        w = torch.tensor(class_weights, dtype=torch.float32, device=x.device)  # [B]
        w = w / (w.sum() + 1e-8)
        idx = torch.multinomial(w, B, replacement=True)
    x2 = x[idx]
    # mix
    x_mix = lam * x + (1 - lam) * x2
    # label: hard-label 유지(주 레이블 y). (대안: soft label CE, but focal/hard 사용)
    return x_mix, y

def gaussian_noise(x, sigma=0.03):
    if sigma <= 0: return x
...
def train_one_fold(X_tr, y_tr, X_te, y_te, files_te, pids_te,
                   ...
                   mixup_alpha=0.3, noise_sigma=0.03, minority_bias=True):
    ...
    # alpha for focal = 1/freq (train-internal)
    ...
    # class-aware partner sampling weights for mixup (minority upweight)
    if minority_bias:
        freq = np.array([cnt.get(i, 1) for i in range(n_class)], dtype=np.float32)
        inv = 1.0 / freq
        inv = inv / inv.sum()
        # per-sample weight = inv[class_of_sample]
        per_sample_w = inv[y_trn]  # mapped later per batch index
    else:
        per_sample_w = None
    ...
    for ep in range(1, epochs+1):
        ...
        for xb, yb, _, _ in dl_trn:
            ...
            # (2) mixup — class-aware: partner draw prob ∝ 1/freq(class)
            if minority_bias:
                # per-sample weights for this batch (take from global vector by matching labels)
                b_weights = torch.tensor([per_sample_w[(y.item())] for y in yb], dtype=torch.float32, device=device)
                xb_aug, yb_aug = mixup_batch(xb_aug, yb, alpha=mixup_alpha, class_weights=b_weights)
            else:
                xb_aug, yb_aug = mixup_batch(xb_aug, yb, alpha=mixup_alpha, class_weights=None)
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       title="MLP + Focal + FeatureAug (Aggregate)")
plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, "MLP_focal_featureaug_cm_aggregate.png"), dpi=150); plt.close(fig)
...
pred_df.to_csv(os.path.join(RESULTS_DIR, "MLP_focal_featureaug_cv_predictions.csv"), index=False)
...
pd.DataFrame([{
    'model': 'MLP(256)+Focal(g=1.0)+FeatureAug(mixup0.3,noise0.03,minority-biased)+Sampler',
