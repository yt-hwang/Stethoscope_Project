...
RESULTS_DIR = os.path.join(PROJECT_DIR, "results", "MLP_Focal")
FIGURES_DIR = os.path.join(RESULTS_DIR, "figures")
os.makedirs(RESULTS_DIR, exist_ok=True); os.makedirs(FIGURES_DIR, exist_ok=True)


CSV_PATH = os.path.join(FEATURES_DIR, "opera_features.csv")
print("[RUN] Step5 MLP Focal"); print(f"[PATH] {CSV_PATH}")
...
def train_one_fold(X_tr, y_tr, X_te, y_te, files_te, pids_te,
                   epochs=50, batch=64, lr=1e-3, gamma=1.0, weight_decay=1e-4):
    ...
    # train 내부에서 소규모 patient-based val(early stop)
    gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    tr_idx, va_idx = next(gss.split(X_tr, y_tr, groups=pids_te[:len(X_tr)] if False else np.arange(len(X_tr))))
    # ↑ 그룹 라벨이 없으므로 간단히 셔플 분할(outer가 patient-based라 누수 위험 낮음)

    X_trn, y_trn = X_tr[tr_idx], y_tr[tr_idx]
    X_val, y_val = X_tr[va_idx], y_tr[va_idx]

    # α = 1/freq (train-internal)
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       title="MLP + FocalLoss (Aggregate)")
plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, "MLP_focal_cm_aggregate.png"), dpi=150); plt.close(fig)
...
pred_df.to_csv(os.path.join(RESULTS_DIR, "MLP_focal_cv_predictions.csv"), index=False)
...
pd.DataFrame([{
    "model": "MLP(hidden=256, drop=0.3) + Focal(gamma=1.0) + Sampler",
