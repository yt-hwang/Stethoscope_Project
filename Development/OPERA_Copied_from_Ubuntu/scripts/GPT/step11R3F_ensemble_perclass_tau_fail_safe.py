...
"""
Step11R3F — Ensemble(LogReg+MLP) + Per-class Threshold + Fail-safe
- 하드 게이트 제거
- NB guard 완화 루프 (0.35 → 0.05, step=0.05)
- 여전히 실패 시 최후 폴백: NB_tau=0.05, H/W/R=0.50, guard 미적용으로 macro 최대 조합 채택
- 앙상블 가중치도 검증에서 함께 탐색 {(0.5,0.5),(0.6,0.4),(0.7,0.3),(0.8,0.2)}
"""
...
RESULTS_DIR  = os.path.join(PROJECT_DIR, 'results', 'Ensemble_PerClassTau_FailSafe')
...
print('[RUN] Step11R3F Ensemble + Per-class τ + Fail-safe')
...
# ---------------- CV with per-class τ tuning + fail-safe ----------------
...
for tr, te in gkf.split(X_all, y_all, groups_all):
    ...
    P_lr_val  = infer_lr(X_val)
    P_mlp_val = infer_mlp(X_val)
    ...
    # target guard: 시작은 max(baseline, 0.35) → 실패 시 0.30, 0.25, ... , 0.05
    ...
    for guard in guard_targets:
        ...
        for w_lr, w_mlp in w_cands:
            ...
            if not nb_ok:
                # guard 완화 루프 안에서라면 다음 guard로 넘어가며 계속 시도
                continue

            # 2) 나머지 클래스 τ 탐색 (NB는 위에서 선택값 고정)
            for tH in tau_vals:
                for tW in tau_vals:
                    for tR in tau_vals:
                        taus = np.full(len(class_names), 0.5, np.float32)
                        taus[IDX_NB] = nb_tau_sel
                        taus[IDX_H]  = tH
                        taus[IDX_W]  = tW
                        taus[IDX_R]  = tR
                        y_hat = predict_with_perclass_tau(P_val, taus)
    ...
    P_lr_te  = infer_lr_full(X_te)
    P_mlp_te = infer_mlp_full(X_te)
    w_lr, w_mlp = best[1]
...
print("\n--- Aggregate Classification Report (Ensemble + Per-class τ + Fail-safe) ---")
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       title='Ensemble + Per-class τ + Fail-safe (Aggregate)')
plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, 'ensemble_perclass_tau_failsafe_cm.png'), dpi=150); plt.close(fig)
...
pd.DataFrame([{
    'model': 'Ensemble(LogReg+MLP)+PerClassTau+FailSafe',
