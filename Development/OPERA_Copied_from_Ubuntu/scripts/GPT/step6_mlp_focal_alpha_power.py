...
RESULTS_DIR  = os.path.join(PROJECT_DIR, 'results', 'MLP_Focal_AlphaPower')
...
print('[RUN] Step6 MLP Focal (alpha^p)'); print(f'[PATH] {CSV_PATH}')
...
def train_one_fold(X_tr, y_tr, X_te, y_te, files_te, pids_te,
                   epochs=50, batch=64, lr=1e-3, gamma=1.0, alpha_power=1.5, weight_decay=1e-4):
    ...
    # 내부 밸리데이션(간단 셔플 분할; outer가 patient-based라 누수 위험 낮음)
    ...
    print(f"[ALPHA] power={alpha_power} → {alpha.round(3)}")

    # Sampler: 여전히 1/freq (훈련 표본 균형 유지)
    weights = np.array([1.0 / max(cnt[c],1) for c in y_trn], dtype=np.float32)
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       title='MLP + Focal(alpha^1.5) (Aggregate)')
plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, 'MLP_focal_alphaP_cm_aggregate.png'), dpi=150); plt.close(fig)
...
pred_df.to_csv(os.path.join(RESULTS_DIR, 'MLP_focal_alphaP_cv_predictions.csv'), index=False)
...
pd.DataFrame([{
    'model': 'MLP(256, drop=0.3) + Focal(gamma=1.0, alpha~(1/freq)^1.5) + Sampler',
