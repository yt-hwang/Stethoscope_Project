...
# Step14 — Class-Balanced Focal(MLP) + Focal(LogReg) → Temp Scaling → Ensemble → Per-class τ
# PROJECT_DIR 고정
...
RESULTS_DIR  = os.path.join(PROJECT_DIR, 'results', 'Step14_CBfocal_PerClassTau')
...
print('[RUN] Step14 CB-Focal → per-class τ')
...
def effective_num_alpha(y, n_class, beta=0.999):
    # CB-Focal alpha from "Class-Balanced Loss Based on Effective Number of Samples"
    cnt = Counter(y.tolist())
    ef = []
    for c in range(n_class):
        n = cnt.get(c, 0)
        if n<=0: ef.append(0.0)
        else:
            ef.append((1 - beta) / (1 - beta**n))
    a = np.array(ef, np.float32)
    a = a / (a.sum()/n_class + 1e-12)  # normalize around 1
    return a
...
# --------- training wrappers ----------
def train_model(model_type, X_tr, y_tr, batch=64, epochs=50, lr=1e-3, wd=1e-4, use_cb_focal=False):
    n_class = len(class_names)
    scaler = StandardScaler()
    X_tr = scaler.fit_transform(X_tr).astype(np.float32)

    if use_cb_focal:
        alpha_np = effective_num_alpha(torch.tensor(y_tr), n_class, beta=0.999)
    else:
    ...
    if model_type=='lr':
        ...
        # CB-Focal here
        crit = FocalLoss(alpha=alpha, gamma=1.5 if use_cb_focal else 1.0)
...
def ensemble_probs(P_lr, P_mlp, w_lr=0.5, w_mlp=0.5): return w_lr*P_lr + w_mlp*P_mlp
...
for tr, te in gkf.split(X_all, y_all, groups_all):
    ...
    # train LR (focal) + MLP (CB-Focal)
    infer_lr  = train_model('lr',  X_trn, y_trn, epochs=30, lr=1e-3, wd=0.0,   use_cb_focal=False)
    infer_mlp = train_model('mlp', X_trn, y_trn, epochs=50, lr=1e-3, wd=1e-4, use_cb_focal=True)

    L_lr_val  = infer_lr(X_val);  L_mlp_val = infer_mlp(X_val)
    ...
    L_lr_val_T, L_mlp_val_T = apply_T_to_logits(L_lr_val, T_lr), apply_T_to_logits(L_mlp_val, T_mlp)
    print(f"[TEMP] T_lr={T_lr:.3f}, T_mlp={T_mlp:.3f}")

    P_lr_val, P_mlp_val = softmax_np(L_lr_val_T), softmax_np(L_mlp_val_T)
    base = ensemble_probs(P_lr_val, P_mlp_val, 0.5, 0.5).argmax(axis=1)
    base_nb = recall_score(y_val, base, labels=[IDX_NB], average=None, zero_division=0)[0]
    base_macro = recall_score(y_val, base, average='macro', zero_division=0)
    print(f"[VAL BASE] macro={base_macro:.3f} NB_recall={base_nb:.3f}")

    # τ / weight grid with guard ladder
    ...
    (w_lr,w_mlp), taus = best[1], best[2]

    # retrain on full-train → test
    infer_lr_full  = train_model('lr',  X_tr, y_tr, epochs=30, lr=1e-3, wd=0.0,   use_cb_focal=False)
    infer_mlp_full = train_model('mlp', X_tr, y_tr, epochs=50, lr=1e-3, wd=1e-4, use_cb_focal=True)
    L_lr_te  = infer_lr_full(X_te);  L_mlp_te = infer_mlp_full(X_te)
    L_lr_te_T, L_mlp_te_T = apply_T_to_logits(L_lr_te, T_lr), apply_T_to_logits(L_mlp_te, T_mlp)
    P_te = ensemble_probs(softmax_np(L_lr_te_T), softmax_np(L_mlp_te_T), w_lr, w_mlp)
    ...
    TAUS.append(taus); WEIGHTS.append((w_lr,w_mlp)); TEMPS.append((T_lr,T_mlp))

# -------- Aggregate --------
T = np.concatenate(ALL_T); P = np.concatenate(ALL_P); PR = np.vstack(ALL_PR)
print("\n--- Aggregate Classification Report (Step14) ---")
...
ax.set(xticks=np.arange(len(class_names)), yticks=np.arange(len(class_names)),
       ...
       title='Step14 (CB-Focal + Temp + Per-class τ) — Aggregate')
plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
for (i,j), v in np.ndenumerate(cm): ax.text(j, i, str(v), ha='center', va='center')
fig.tight_layout(); fig.savefig(os.path.join(FIGURES_DIR, 'step14_cm.png'), dpi=150); plt.close(fig)
...
pd.DataFrame([{
    'model': 'Step14_CBfocal_PerClassTau',
